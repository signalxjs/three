...
function main() {
    ...
    writeFileSync(
        ...
        [
            ...
            '',
            '// The consumer shape: HTML and three elements in one file, on the',
            '// published .d.ts chain (proves the global JSX merge survives packing).',
            'const Cube = component(() => {',
            '    const mesh = objectRef<Mesh>();',
            '    const x = signal(0);',
            '    useFrame((_, dt) => { if (mesh.current) mesh.current.rotation.y += dt; });',
            '    return () => (',
            '        <mesh ref={mesh} position-x={x} onClick={(e) => e.stopPropagation()}>',
            ...
            '            Count: {count.value}',
            '            <Canvas camera={{ position: [0, 0, 4] }}><Cube /></Canvas>',
            '        </div>',
            '    );',
            '});',
            '',
